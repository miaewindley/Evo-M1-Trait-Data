...
#
# NOTE: this .R is the house-style reproducible equivalent of build_metabolic_merge.py, which is the
...
setwd("~/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo-M1-Trait-Data/__merging_metabolic")
...
U %>% select(Species,Species_printed,Compilation,Table,Region,Region_raw,Measure,Value,SD,n,Units,conscious,ref_raw,ref_keys_str) %>%
  arrange(Measure,Species,Region,Compilation) %>% write_csv("metabolic_unfiltered.csv")
...
collide %>% group_by(Species,Region,Measure,shared_ref=ref_keys) %>%
  ...
  arrange(Species,Region,Measure) %>% write_csv("metabolic_dedupe_report.csv")
...
write_csv(merged, "metabolic_long.csv")

## ---- 6. wide -----------------------------------------------------------------------------------
merged %>% mutate(col=paste0(Region,"__",Measure)) %>% select(Species,col,Value) %>%
  pivot_wider(names_from=col, values_from=Value) %>% arrange(Species) %>% write_csv("metabolic_wide.csv")
...
U %>% count(Species,Species_printed,Compilation) %>%
  ...
  arrange(Species,Species_printed) %>% write_csv("metabolic_source_species_ids.csv")
