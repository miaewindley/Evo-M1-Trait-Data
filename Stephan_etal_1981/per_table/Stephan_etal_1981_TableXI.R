...
#
# Table XI. Volumes of palaeocortex, of amygdala and its various components (mm3) in primates
...
#
# Input : per_table/Stephan_etal_1981_TableXI_snapshot.xlsx
# Output: Stephan_etal_1981_TableXI.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableXI_snapshot.xlsx"),
                   sheet = "TableXI", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Palaeocortex, Amygdala, Complexus_centromedialis, Nucleus_tractus_olfactorius, Complexus_corticobasolateralis, Nucleus_amygdalae_basalis_pars_magnocellularis) %>%
  mutate(source = "Stephan_etal_1981_TableXI")
...
item_encoded <- "10.1159%2F000155963_TableXI"
if (!is.na(base)) {
