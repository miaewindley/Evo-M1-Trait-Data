...
#
# Table I. Data on body weights (g), brain weights (mg), and volumes of total brain and fundamental brain parts (mm3) in insectivores
...
#
# Input : per_table/Stephan_etal_1981_TableI_snapshot.xlsx
# Output: Stephan_etal_1981_TableI.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableI_snapshot.xlsx"),
                   sheet = "TableI", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableI")
...
item_encoded <- "10.1159%2F000155963_TableI"
if (!is.na(base)) {
