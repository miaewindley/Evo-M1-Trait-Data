...
#
# Table VII. Volumes of components of the diencephalon (mm3) in insectivores
...
#
# Input : per_table/Stephan_etal_1981_TableVII_snapshot.xlsx
# Output: Stephan_etal_1981_TableVII.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableVII_snapshot.xlsx"),
                   sheet = "TableVII", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableVII")
...
item_encoded <- "10.1159%2F000155963_TableVII"
if (!is.na(base)) {
