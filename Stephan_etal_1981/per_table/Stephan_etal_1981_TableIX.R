...
#
# Table IX. Volumes of visual cortex (area striata) and lateral geniculate body (mm3) in primates
...
#
# Input : per_table/Stephan_etal_1981_TableIX_snapshot.xlsx
# Output: Stephan_etal_1981_TableIX.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableIX_snapshot.xlsx"),
                   sheet = "TableIX", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Area_striata, Corpus_geniculatum_laterale) %>%
  mutate(source = "Stephan_etal_1981_TableIX")
...
item_encoded <- "10.1159%2F000155963_TableIX"
if (!is.na(base)) {
