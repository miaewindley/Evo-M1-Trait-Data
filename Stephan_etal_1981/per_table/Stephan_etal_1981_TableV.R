...
#
# Table V. Volumes of the telencephalon and of its components (mm3) in prosimians
...
#
# Input : per_table/Stephan_etal_1981_TableV_snapshot.xlsx
# Output: Stephan_etal_1981_TableV.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableV_snapshot.xlsx"),
                   sheet = "TableV", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableV")
...
item_encoded <- "10.1159%2F000155963_TableV"
if (!is.na(base)) {
