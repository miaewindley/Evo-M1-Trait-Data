...
#
# Table III. Data on body weights (g), brain weights (mg), and volumes of total brain and fundamental brain parts (mm3) in simians
# Taxon scope: Simian. All volumes in mm3; body weight g; brain weight mg.
#
...
#
# Input : per_table/Stephan_etal_1981_TableIII_snapshot.xlsx
# Output: Stephan_etal_1981_TableIII.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableIII_snapshot.xlsx"),
                   sheet = "TableIII", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Body_weight, Brain_weight, Ventricles, Meninges_hypophysis_nerves_etc., Total_brain_net_volume, Medulla_oblongata, Cerebellum, Mesencephalon, Diencephalon, Telencephalon) %>%
  mutate(source = "Stephan_etal_1981_TableIII")
...
item_encoded <- "10.1159%2F000155963_TableIII"
if (!is.na(base)) {
