...
#
# Table XIV. Volumes of various periventricular structures (mm3) in insectivores
...
#
# Input : per_table/Stephan_etal_1981_TableXIV_snapshot.xlsx
# Output: Stephan_etal_1981_TableXIV.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableXIV_snapshot.xlsx"),
                   sheet = "TableXIV", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableXIV")
...
item_encoded <- "10.1159%2F000155963_TableXIV"
if (!is.na(base)) {
