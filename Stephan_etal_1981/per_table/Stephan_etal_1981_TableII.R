...
#
# Table II. Data on body weights (g), brain weights (mg), and volumes of total brain and fundamental brain parts (mm3) in prosimians
# Taxon scope: Prosimian. All volumes in mm3; body weight g; brain weight mg.
#
...
#
# Input : per_table/Stephan_etal_1981_TableII_snapshot.xlsx
# Output: Stephan_etal_1981_TableII.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableII_snapshot.xlsx"),
                   sheet = "TableII", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableII")
...
item_encoded <- "10.1159%2F000155963_TableII"
if (!is.na(base)) {
