...
#
# Table XV. Volumes of various periventricular structures (mm3) in prosimians
...
#
# Input : per_table/Stephan_etal_1981_TableXV_snapshot.xlsx
# Output: Stephan_etal_1981_TableXV.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableXV_snapshot.xlsx"),
                   sheet = "TableXV", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableXV")
...
item_encoded <- "10.1159%2F000155963_TableXV"
if (!is.na(base)) {
