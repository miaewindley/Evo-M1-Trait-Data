...
#
# Table XVI. Volumes of various periventricular structures (mm3) in simians
...
#
# Input : per_table/Stephan_etal_1981_TableXVI_snapshot.xlsx
# Output: Stephan_etal_1981_TableXVI.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableXVI_snapshot.xlsx"),
                   sheet = "TableXVI", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Nucleus_septalis_triangularis, Corpus_subfornicale, Nucleus_habenularis_medialis, Corpus_pineal, Corpus_subcommissurale) %>%
  mutate(source = "Stephan_etal_1981_TableXVI")
...
item_encoded <- "10.1159%2F000155963_TableXVI"
if (!is.na(base)) {
