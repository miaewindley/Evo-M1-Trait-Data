...
#
# Table XIII. Volumes of vestibular complex and its various components (mm3) in primates
# Taxon scope: Prosimian+Simian. All volumes in mm3; body weight g; brain weight mg.
#
# Laterality: Baron et al. 1988 states the Stephan 1981 vestibular volumes (codes 35-39)
# were measured from ONE side only. Values are left unchanged; the five columns get a
# _unilateral suffix so a one-side value is never silently averaged with a both-sides one.
#
...
#
# Input : per_table/Stephan_etal_1981_TableXIII_snapshot.xlsx
# Output: Stephan_etal_1981_TableXIII.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableXIII_snapshot.xlsx"),
                   sheet = "TableXIII", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Complexus_vestibularis_unilateral, Nucleus_vestibularis_superior_unilateral, Nucleus_vestibularis_lateralis_unilateral, Nucleus_vestibularis_medialis_unilateral, Nucleus_vestibularis_descendens_unilateral) %>%
  mutate(source = "Stephan_etal_1981_TableXIII")

# apply _unilateral suffix (see laterality note above)
uni <- c(
...
nm <- names(clean); hit <- nm %in% names(uni); names(clean)[hit] <- unname(uni[nm[hit]])
...
item_encoded <- "10.1159%2F000155963_TableXIII"
if (!is.na(base)) {
