...
#
# Table X. Volumes of palaeocortex, of amygdala and its various components (mm3) in insectivores
...
#
# Input : per_table/Stephan_etal_1981_TableX_snapshot.xlsx
# Output: Stephan_etal_1981_TableX.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableX_snapshot.xlsx"),
                   sheet = "TableX", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  mutate(source = "Stephan_etal_1981_TableX")
...
item_encoded <- "10.1159%2F000155963_TableX"
if (!is.na(base)) {
