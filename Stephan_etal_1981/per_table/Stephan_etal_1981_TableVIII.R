...
#
# Table VIII. Volumes of components of the diencephalon (mm3) in primates
...
#
# Input : per_table/Stephan_etal_1981_TableVIII_snapshot.xlsx
# Output: Stephan_etal_1981_TableVIII.csv  +  DOI-encoded TSV in __Public/comparative-data/
...
snap <- read_excel(file.path(folder, "Stephan_etal_1981_TableVIII_snapshot.xlsx"),
                   sheet = "TableVIII", skip = 1)   # row 1 is the caption
...
clean <- snap %>%
  ...
  transmute(species, group, Epithalamus, Thalamus, Hypothalamus, Subthalamus, Pallidum, Nucleus_subthalamicus, Capsula_interna, Tractus_opticus) %>%
  mutate(source = "Stephan_etal_1981_TableVIII")
...
item_encoded <- "10.1159%2F000155963_TableVIII"
if (!is.na(base)) {
