...
library(readxl)
tabledirectxl <- read_excel("HerculanoHouzel_etal_2015_Table1_primary_or_equivalent.xlsx")
...
write.csv(tabledirectxl, file = "~/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo M1 Trait Data/__Public/comparative-data/10.1159%2F000437413_Table1.tsv", row.names = FALSE)
...
new_dataframe <- data.frame(
  ...
  Reference = rep("HerculanoHouzel_etal_2015_Table1", ncol(tabledirectxl)),  # Reference column
...
file_path <- "~/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo M1 Trait Data/__merging/HerculanoHouzel_etal_2015_Table1_terms.csv"
