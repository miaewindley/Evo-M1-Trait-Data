...
- **Worksheet:** `Group Size Data (aggregated)`
- **Rows written:** 72
- **Role:** Species-level mean group size values and source reference numbers used by the study.
...
- **Reformat:** `DeCasien_Higham_2019_GroupSizeDataaggregated.R` reads only this worksheet, removes only wholly empty formatting rows/columns, preserves source fields and writes:
  - `DeCasien_Higham_2019_GroupSizeDataaggregated.csv`
  - `__Public/comparative-data/10.1038%2Fs41559-019-0969-0_GroupSizeDataaggregated.tsv` when no registry encoding is available.
