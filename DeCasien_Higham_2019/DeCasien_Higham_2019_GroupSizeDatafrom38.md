# DeCasien & Higham 2019 - Group Size Data (from 38)
...
- **Worksheet:** `Group Size Data (from 38)`
- **Rows written:** 936
- **Role:** Underlying group-size records from reference 38, including duplicate flags, immediate sources and notes.
- **Taxonomic coverage:** 157 distinct printed `Taxon` values; printed names are preserved and `species_sci` is added through the project species key.

## What we built

- **Frozen source (digital-native, no derived snapshot):** the journal workbook is retained verbatim and read directly.
- **Reformat:** `DeCasien_Higham_2019_GroupSizeDatafrom38.R` reads only this worksheet, removes only wholly empty formatting rows/columns, preserves source fields and writes:
  - `DeCasien_Higham_2019_GroupSizeDatafrom38.csv`
  - `__Public/comparative-data/10.1038%2Fs41559-019-0969-0_GroupSizeDatafrom38.tsv` when no registry encoding is available.
