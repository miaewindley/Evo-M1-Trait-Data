# Volume merge — health check + DeCasien expansion (2026-06-24)
...
Two requests: (1) confirm the volume merge is up and working; (2) expand it to include
...
## 2. Expansion with DeCasien data — done
...
| status | original merge | expanded merge |
|---|---|---|
...
## Files
- **Canonical outputs now hold the expanded, real-Sherwood data** (34 tables, 282 species):
  `volumes_long.csv`, `volumes_wide.csv`, `volumes_flags.csv`, `volumes_source_species_ids.csv`.
  Identical `*_EXPANDED.csv` copies are kept for reference. (A prior R run had produced the merge
  using the placeholder viaDeCasien Sherwood; these files now reflect the real primary.)
...
- `DeCasien_vs_merge_comparison_EXPANDED.csv` — per-cell baseline-vs-expanded status.
