# EXPANDED volume merge = canonical 30 + DeCasien-comparison sources, for merge-vs-DeCasien.
# DEDICATED comparison/expansion script — NOT the canonical dataset (that is volumes_compiled.R).
# It ADDS the DeCasien-overlapping papers, writes *_EXPANDED.csv (never the canonical volumes_*.csv),
# then runs the merge-vs-DeCasien value comparison. ENGINE MIRRORS volumes_compiled.R — keep in sync;
# the only differences are +7 DeCasien rows, +4 reshapes, +7 enc_override fallbacks, *_EXPANDED
# outputs, and the step-9 comparison call at the bottom.
#
...
# Dedicated output suffix for this expansion script. Keep every file written here
# separate from the canonical volumes_compiled.R outputs.
output_suffix <- "_EXPANDED"
expanded_csv  <- function(stem) paste0(stem, output_suffix, ".csv")
...
papers <- tribble(
  ...
  "MacLeod_etal_2003_",                   "Zilles",             2003,
  ...
  "Semendeferi_etal_1998_Table2",         "Semendeferi",        1998,
  "Semendeferi_etal_2001_Table2",         "Semendeferi",        2001,
  "Sherwood_etal_2005_Table1",            "Sherwood",           2005,
  "Barger_etal_2007_TABLE1",              "Zilles",             2007,

  # --- DeCasien-comparison sources: overlapping with, but NOT identical to, the canonical 30 ---
  # Real primaries (own species reshape in step 3; species resolved in step 4 like everything else):
  "Sherwood_etal_2004_TABLEI",            "Sherwood",           2004,
  "Barks_etal_2014_TABLE1",               "Barks",              2014,
  "Rilling_Insel_1998_Table1",            "RillingInsel",       1998,
  "Stimpson_etal_2015_TableS1",           "Stimpson",           2015,
  # DeCasien-extracted (MOESM3); NB 62-63 NEOCORTEX is Rilling & Insel 1999 (ref 63), not 1998.
  "Barks_etal_2014_viaDeCasien",          "Barks",              2014,
  "Rilling_Insel_1999_viaDeCasien",       "RillingInsel",       1999,
  "Stimpson_etal_2015_viaDeCasien",       "Stimpson",           2015
  # NOTE: DeCasien rows ARE compiled here (this IS the expansion). The canonical 30-table merge is
  # volumes_compiled.R; this script writes *_EXPANDED.csv and runs the DeCasien comparison at the end.
)
...
ncbi_pref <- taxid2name(ncbi_ids$id, out_type = "summary")
ncbi <- tibble(Species_raw = uniq,
               NCBI_id   = ncbi_ids$id[match(uniq, ncbi_ids$name)]) %>%
        mutate(NCBI_name = ncbi_pref$name[match(NCBI_id, ncbi_pref$id)])
...
resolved <- raw %>%
  ...
  select(-key)
write_csv(resolved %>% arrange(Source, Species_raw), expanded_csv("volumes_source_species_ids"))
if (any(resolved$flag_unresolved))
  warning("Species resolution: ", sum(resolved$flag_unresolved), " (source, name) pair(s) had no ",
          "curated override and no NCBI match -> kept raw. See volumes_source_species_ids_EXPANDED.csv.")
...
write_csv(long, expanded_csv("volumes_unfiltered"))
...
write_csv(flags, expanded_csv("volumes_flags"))
...
write_csv(volumes_long, expanded_csv("volumes_long"))
...
## recorded in volumes_flags_EXPANDED.csv (flag = estimated_bilateral_from_unilateral).
...
write_csv(volumes_wide, expanded_csv("volumes_wide"))
...
long %>% group_by(Species_Name = Species) %>% summarise(n_sources=n_distinct(Source), Sources=paste(sort(unique(Source)),collapse="; ")) %>%
  write_csv(expanded_csv("volumes_species_sources"))

message("[EXPANDED] ", nrow(volumes_wide), " species x ", ncol(volumes_wide)-1, " variables from ",
        nrow(papers), " tables | flags: ", nrow(flags), " | outputs: volumes_*_EXPANDED.csv")

## 9 Compare this EXPANDED merge against DeCasien & Higham 2019 ----
## Reuse the maintained value-match comparison (no duplicated logic), pointed at the EXPANDED outputs.
## Writes DeCasien_Higham_2019/DeCasien_vs_merge_comparison_EXPANDED.csv (+ _EXPANDED findings/taxonomy).
