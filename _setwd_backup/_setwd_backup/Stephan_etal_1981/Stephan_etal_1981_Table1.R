...
if (requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::isAvailable())
  setwd("/Users/crossmodal/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo-M1-Trait-Data/_setwd_backup/_setwd_backup/Stephan_etal_1981")
...
dat <- dplyr::rename(dat, Species_Stephan1981 = species)

# type every column except the two identifiers
id <- c("code", "Species_Stephan1981")
final.dataframe <- dat %>%
  mutate(across(-all_of(id), num)) %>%
  mutate(code = str_squish(code), Species_Stephan1981 = str_squish(Species_Stephan1981))
