# Matano_etal_1985_b_Table1_compare_to_Matano_1985b_csv.R
#
...
# Run from comparison/.
#
# Inputs : ../Matano_etal_1985_b_Table1_snapshot.xlsx (sheet Table1) ; Matano_1985b.csv
# Outputs: Matano_etal_1985_b_Table1_comparison_report_from_R.csv
#          Matano_etal_1985_b_Table1_comparison_mismatches_from_R.csv

suppressPackageStartupMessages({
  library(readxl); library(readr); library(dplyr); library(tidyr); library(stringr)
})
if (requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::isAvailable())
  setwd("/Users/crossmodal/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo-M1-Trait-Data/_setwd_backup/_setwd_backup/Matano_etal_1985_b/comparison")

snapshot_file     <- "../Matano_etal_1985_b_Table1_snapshot.xlsx"
snapshot_sheet    <- "Table1"
comparison_file   <- "Matano_1985b.csv"
output_detail     <- "Matano_etal_1985_b_Table1_comparison_report_from_R.csv"
output_mismatches <- "Matano_etal_1985_b_Table1_comparison_mismatches_from_R.csv"
header_rows       <- 3L
structures <- c("ventral_pons","body_weight","n")
csv_col <- c(ventral_pons = "Ventral_pons", body_weight = "Body_weight_1985b",
             n = "Number_Ventral_pons_1985b")
...
pos <- c("code","species_disp","n_raw","body_weight","ventral_pons")
...
snap <- sdat %>%
  filter(!is.na(parse_value(ventral_pons))) %>%
  transmute(species_key = norm_label(species_disp), species_snapshot = str_squish(species_disp),
            ventral_pons_snap = parse_value(ventral_pons),
            body_weight_snap  = parse_value(body_weight),
...
message("matched: ", sum(report$status == "matched_by_species"),
        ...
        "  (Pongo VPo has no paper species name -> expected csv-only; a pre-1988 addition)")
