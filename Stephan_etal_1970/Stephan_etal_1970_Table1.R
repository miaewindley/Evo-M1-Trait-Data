...
#
# Table 1. Body weight, brain weight, total brain net volume and fundamental brain parts (mm3) in insectivores
# Taxon group: Insectivore. All volumes in mm3; body weight in g; brain weight in mg.
#
...
#
# Input : Stephan_etal_1970_Table1_snapshot.csv
# Output: Stephan_etal_1970_Table1.csv  +  ISBN-encoded TSV in __Public/comparative-data/
...
snap <- read_csv(file.path(folder, "Stephan_etal_1970_Table1_snapshot.csv"), show_col_types = FALSE)
...
clean <- snap %>%
  transmute(species, group,
            ...
            source = "Stephan_etal_1970_Table1")
...
item_encoded <- "ISBN%3A0390672505_TABLE1"
if (!is.na(base)) {
