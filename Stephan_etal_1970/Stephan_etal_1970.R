# Stephan, Bauchot & Andy 1970 - full reformat (Tables 1-6)  [DRAFT, pending verification]
...
# text layer and the figshare xlsx are corrupted OCR). DRAFT - verify against the PDF.

library(tidyverse)
base <- "~/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo-M1-Trait-Data"
folder <- file.path(base, "Stephan_etal_1970")

t13 <- read_csv(file.path(folder, "Stephan_etal_1970_Tables1-3_snapshot_DRAFT.csv"), show_col_types = FALSE)
t46 <- read_csv(file.path(folder, "Stephan_etal_1970_Tables4-6_snapshot_DRAFT.csv"), show_col_types = FALSE)
...
message("Stephan 1970 Tables 1-6 (DRAFT): ", nrow(clean), " species.")
