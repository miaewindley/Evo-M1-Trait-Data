...
# The snapshot's FIRST line is the column header (Species, Frontal_GM_L_cm3, ...). Read it
# directly — do NOT skip a line. (An earlier snapshot carried a title row above the header, so
# this used skip = 1; the current snapshot starts at the header, and skipping it would promote
# the first data row to column names, e.g. a "Gorilla beringei" / "1.0...16" garbage header.)
raw <- read_csv(snapshot_csv, col_names = TRUE, show_col_types = FALSE) %>%
  select(where(~ !all(is.na(.x))))
if (!"Species" %in% names(raw))
  stop("Barks_etal_2014_Fig4A: expected a 'Species' column header on line 1 of the snapshot; got: ",
       paste(head(names(raw), 4), collapse = ", "), call. = FALSE)
