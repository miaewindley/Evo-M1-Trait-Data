#!/usr/bin/env python3
"""
Build the __merging_cerebral_metabolic_rate compilation-aware merge.
...
cerebral_metabolic_rate_compiled.R / standardized_term.R are shipped as the house-style reproducible
...
OUT  = os.path.join(BASE, "__merging_cerebral_metabolic_rate")
...
U_out.to_csv(os.path.join(OUT, "cerebral_metabolic_rate_unfiltered.csv"), index=False)
...
dedupe_report.to_csv(os.path.join(OUT, "cerebral_metabolic_rate_dedupe_report.csv"), index=False)
...
merged.to_csv(os.path.join(OUT, "cerebral_metabolic_rate_long.csv"), index=False)
...
w = merged.assign(col=merged["Region"]+"__"+merged["Measure"]) \
          ...
          .sort_index()
w.to_csv(os.path.join(OUT, "cerebral_metabolic_rate_wide.csv"))
...
sid_wide.to_csv(os.path.join(OUT, "cerebral_metabolic_rate_source_species_ids.csv"), index=False)
...
#    + stacked standardized_term_cerebral_metabolic_rate.csv
# ----------------------------------------------------------------------------------
...
pd.concat(term_frames, ignore_index=True).to_csv(os.path.join(OUT,"standardized_term_cerebral_metabolic_rate.csv"), index=False)
print("[terms] wrote per-reference standardized-term files + stacked standardized_term_cerebral_metabolic_rate.csv")

print("\nDONE. Outputs in __merging_cerebral_metabolic_rate/")
